...
class IntelligentHotelAgent:
    ...
    def extract_dates_from_message(self, message):
        ...
        date_patterns = re.findall(r'\b\d{4}-\d{2}-\d{2}\b', message)
        ...
        all_dates = date_patterns + relative_dates
    ...
    def process_guest_booking(self, booking_state):
        ...
        try:
            ...
            if booking_response.status_code == 200:
                ...
                self.update_room_with_guest_info(
        ...
        try:
            ...
            if st.session_state.get('date_collection_step'):
                ...
                if st.session_state['date_collection_step'] == 'check_in' and dates:
                    ...
                    return self.handle_room_availability(f"Show available rooms from {st.session_state['pending_check_in']} to {dates[0]}", context)
            ...
            # 4. GUEST PROFILE IDENTIFICATION - ONLY FOR PROFILE LOADING
            ...
            # 5. ALL OTHER QUESTIONS - Use Phi-4-mini's knowledge
            else:
...
**Example:** "Check-in: 2025-07-15, Check-out: 2025-07-18"
**Or simply:** "2025-07-15 to 2025-07-18"
...
🏨 **Filtered Results:** Showing only rooms available during your entire stay (excluding reserved rooms)
    ...
    def handle_llm_powered_questions(self, message: str, context: Dict):
        ...
        try:
            ...
            if guest_profile:
                ...
                if any(word in message.lower() for word in ['attractions', 'restaurants', 'places to visit', 'things to do', 'recommend', 'sightseeing', 'dining', 'food']):
                    personalized_prompt = f"""You are a knowledgeable Mumbai hotel concierge speaking to {guest_name}, a {profession} visiting for {purpose}.  

Guest's previous preferences: {amenities_used}, {activities_booked}

The guest is asking: {message}
Provide natural, personalized recommendations for Mumbai based on their profile. Consider their profession, purpose of visit, and previous preferences. Give specific suggestions for:
- Top attractions and landmarks
- Best restaurants and dining experiences  
- Activities suited to their interests
- Local cultural experiences

Make your response conversational and personalized, ending with "Warm regards, Hotel Inn Team".

Keep your response informative but concise, focusing on quality recommendations that match their profile."""

                else:
                    personalized_prompt = f"""You are a helpful Mumbai hotel concierge speaking to {guest_name}, a {profession}.
 Guest question: {message}

Provide a helpful, personalized response as a knowledgeable concierge would. End with 'Warm regards, Hotel Inn Team'."""
                   
            else:
                if any(word in message.lower() for word in ['attractions', 'restaurants', 'places to visit', 'things to do', 'recommend', 'sightseeing', 'dining', 'food']):
                    personalized_prompt = f"""You are a knowledgeable Mumbai hotel concierge. 
 Guest question: {message}
 
Provide excellent local recommendations for Mumbai including:
- Popular attractions and landmarks
- Great restaurants and dining options
- Interesting activities and experiences
- Cultural highlights

Make your response natural and conversational, ending with "Warm regards, Hotel Inn Team"."""

                else:
                    personalized_prompt = f"""You are a helpful Mumbai hotel concierge.
   Guest question: {message}
   Provide a helpful, informative response as a knowledgeable concierge would. End with 'Warm regards, Hotel Inn Team'."""
   
            
            # Generate response using Phi-4-mini
            llm_response = self.ollama_client.generate_response(
                personalized_prompt,
                "You are an experienced Mumbai hotel concierge with deep knowledge of the city. Provide natural, helpful recommendations based on your knowledge. Be conversational and personalized in your responses."
            )
    ...
    def render_interface(self):
        ...
        if st.session_state.get('show_room_table') and st.session_state.get('current_room_data')and st.session_state['mcp_messages'] and len(st.session_state['mcp_messages']) > 0 and 'available rooms' in st.session_state['mcp_messages'][-1].get('content', '').lower():
            st.markdown("---")
            ...
            # Display main table first (before filtering)
            st.dataframe(
                df,
            ...
            if selected_types:
                ...
                if price_filter != "All Prices":
                    # Extract numeric price values properly
                ...
                # Display filtered results only if filters are applied
                filters_applied = (len(selected_types) != len(room_types)) or (price_filter != "All Prices")
                
                if filters_applied:
                    st.subheader(f"🔍 Filtered Results ({len(filtered_df)} rooms)")

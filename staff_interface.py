...
• Booking Server (8002): {'✅' if rooms_response.status_code == 200 else '❌'}"""
    ...
    def generate_room_recommendations_for_staff(self, guest_profile):
        """Generate room recommendations with staff talking points"""
        try:
            # Get available rooms
            ...
            # Analyze available rooms and create recommendations
            for room in available_rooms[:5]:  # Top 5 recommendations
                ...
                if previous_room and room_type == previous_room:
                    talking_points.append(f"• **Perfect Match**: Similar to your previous {previous_room} room experience")
                
                # Loyalty-based recommendations
                if loyalty in ['Silver', 'Gold'] and room_type in ['suite', 'deluxe']:
                    talking_points.append(f"• **VIP Treatment**: As a {loyalty} member, you deserve our premium {room_type}")
                
                # Spending pattern analysis
                if spending > 8000 and price > 5000:
                    talking_points.append(f"• **Value Alignment**: Matches your preference for premium experiences")
                elif spending < 5000 and price < 4000:
                    talking_points.append(f"• **Budget Friendly**: Great value while maintaining quality")
                
                # Special requests accommodation
                if 'extra bed' in special_requests and room_type in ['family', 'suite']:
                    talking_points.append(f"• **Space Solution**: {room_type.title()} room can easily accommodate extra bedding")
                
                if 'medicine' in special_requests:
                    talking_points.append(f"• **Convenience**: Close to front desk for easy access to assistance")
                
                # Amenities matching
                if 'spa' in amenities_used and room_type in ['deluxe', 'suite']:
                    talking_points.append(f"• **Wellness Focus**: Premium rooms include enhanced spa service access")
                
                if 'gym' in amenities_used:
                    talking_points.append(f"• **Fitness Friendly**: Convenient location near fitness facilities")
                
                # Purpose-based recommendations
                if 'business' in purpose and room_type in ['deluxe', 'suite']:
                    talking_points.append(f"• **Business Ready**: Spacious workspace and quiet environment for meetings")
                
                if 'wedding' in purpose and room_type == 'suite':
                    talking_points.append(f"• **Celebration Perfect**: Elegant suite ideal for special occasions")
                
                # Cultural considerations
                if origin and room_type == 'family':
                    talking_points.append(f"• **Family Comfort**: Spacious accommodation perfect for {origin} family traditions")
                
                # Add default talking points if none matched
                if not talking_points:
                    if room_type == 'suite':
                        talking_points.append("• **Luxury Experience**: Our finest accommodation with premium amenities")
                    elif room_type == 'deluxe':
                        talking_points.append("• **Enhanced Comfort**: Upgraded amenities and superior location")
                    elif room_type == 'family':
                        talking_points.append("• **Spacious Living**: Perfect for relaxation and comfort")
                    else:
                        talking_points.append("• **Quality Stay**: Comfortable accommodation with all essential amenities")
                
                recommendations.append({
                    'room_number': room_number,
                    'room_type': room_type.title(),
                    'price': price,
                    'talking_points': talking_points[:3]  # Limit to top 3 points
                })
            
            # Format recommendations for display
            response = "## 🏨 **Personalized Room Recommendations for Staff:**\n\n"
            
            for i, rec in enumerate(recommendations[:3], 1):
                response += f"### **{i}. Room {rec['room_number']} - {rec['room_type']} (₹{rec['price']:,.0f}/night)**\n\n"
                response += "**🗣️ Staff Talking Points:**\n"
                for point in rec['talking_points']:
                    response += f"{point}\n"
                response += "\n**💡 How to present:** *\"Based on your profile and preferences, I believe this room would be perfect because...*\"\n\n"
                response += "---\n\n"
            
            response += """### 🎯 **Additional Upselling Opportunities:**

**🛎️ Services to Mention:**
• Airport pickup service - "Would you like our luxury car service for your arrival?"
• Spa treatments - "We can arrange a relaxing spa session after your journey"
• Room service - "Our 24/7 room service can cater to your specific dietary needs"

**🎭 Activities to Suggest:**
• City tours - "We offer guided tours showcasing local culture and attractions"
• Business center - "Our business facilities are available for any work requirements"
• Special dining - "Our chef can prepare cuisine from your region"

**💰 Revenue Optimization:**
• Bundle services for better value perception
• Emphasize unique experiences over price
• Use guest's previous positive experiences as reference points"""
